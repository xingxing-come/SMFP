...
class CrossModalAttention(nn.Module):
    def __init__(self, d_model=256, h=8, dropout=0.1):
        super().__init__()
        assert d_model % h == 0
        self.d_k = d_model // h
        self.h = h
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # Multi-Head Split
        Q = Q.view(batch_size, -1, self.h, self.d_k).transpose(1, 2)  # [B, h, L_q, d_k]
        K = K.view(batch_size, -1, self.h, self.d_k).transpose(1, 2)  # [B, h, L_k, d_k]
        V = V.view(batch_size, -1, self.h, self.d_k).transpose(1, 2)  # [B, h, L_v, d_k]
...
# Graph-SMILES bidirectional cross-attention fusion
class CrossModalFusion(nn.Module):
    def __init__(self, d_model=256):
        super().__init__()
        self.graph_to_smiles = CrossModalAttention(d_model)
        self.smiles_to_graph = CrossModalAttention(d_model)

    def forward(self, graph_feat, smiles_feat):
        # Enhanced Graph Features (B, L_graph, D)
        graph_enhanced = self.graph_to_smiles(
            query=graph_feat,
            key=smiles_feat,
            value=smiles_feat
        )

        # Enhanced SMILES Features (B, L_smiles, D)
        smiles_enhanced = self.smiles_to_graph(
            query=smiles_feat,
            key=graph_feat,
            value=graph_feat
        )

        #  (B, L_graph+L_smiles, D)
        return torch.cat([graph_enhanced, smiles_enhanced], dim=1)
...
def model_train(model, atom_pred_decoder, smiles_encoder_model,encoder_model ,smiles_decoder_model, train_dataset, model_params, train_params, epochs, experiment_name):
    ...
    cross_modal = CrossModalFusion(d_model=256).to(train_params['device'])

    # build optimizer
    optimizer_cross_modal = ScheduledOptim(
        torch.optim.Adam(cross_modal.parameters(), lr=0),
        train_params['warmup_factor'],
    ...
    if not Path("./Model/" + experiment_name + "/cross_modal/").exists():
        os.makedirs("./Model/" + experiment_name + "/cross_modal/")
    ...
    for epoch in range(epochs):
        ...
        for node_features, bond_features, adjacency_matrix, mask_node_labels, masked_atom_indices, token_ids, labels, edge_attr, edge_index, x, xmasked_atom_indices, xmask_node_labels, num_nodes in tqdm(train_loader):
            ...
            graph_features = node_new_feature + nn.Parameter(torch.zeros(1, 1, 256)).cuda(3)
            token_ids = token_ids.squeeze()

            smiles_embedding = smiles_encoder_model(token_ids)
            smiles_features = smiles_embedding + nn.Parameter(torch.zeros(1, 1, 256)).cuda(3)
            new_embedding = cross_modal(graph_features, smiles_features)
            assert new_embedding.shape[1] == graph_features.shape[1] + smiles_features.shape[1], \
                f": {new_embedding.shape} vs {graph_features.shape[1] + smiles_features.shape[1]}"
            ...
            loss.backward()
            optimizer_cross_modal.step_and_update_lr()
        ...
        #     valid_result['r2'] = metrics.r2_score(valid_result['label'], valid_result['prediction'])
            ...
            if loss < best_valid_loss:
                ...
                torch.save({
                    'state_dict': cross_modal.state_dict(),
                    'best_epoch': epoch,
                    'best_valid_loss': best_valid_loss
                }, f"./Model/{experiment_name}/cross_modal/cross_modal_epoch{epoch}.pth")
...
if __name__ == '__main__':
    ...
    # with open('/home/Desktop/Project/compt/CoMPT/Data/zinc15_250k/zinc15_250K.pickle', 'rb') as f:
    #     mol = pkl.load(f)
